define(['jquery', 'core/log', 'core/ajax', 'core/notification', 'core/str', 'qtype_cloudpoodll/drawing_board'],
    function ($, log, Ajax, Notification, Str, DrawingBoard) {
        ...
        return {
            ...
            setup_strings: function () {
                ...
                // Set up strings
                Str.get_strings([
                    { "key": "save", "component": "qtype_cloudpoodll" },
                    { "key": "saving", "component": "qtype_cloudpoodll" },
                    { "key": "saved", "component": "qtype_cloudpoodll" },
                    { "key": "confirmclearcanvas", "component": "qtype_cloudpoodll" },
                    { "key": "enterthetext", "component": "qtype_cloudpoodll" },
                ]).done(function (s) {
                    var i = 0;
            ...
            setup_controls: function (container, board) {
                ...
                var stampPaths = {
                    ...
                    'star': 'M256 0l79.1 160.3 176.9 25.7-128 124.8 30.2 176.1L256 404.1 99.8 486.9 130 310.8 2 186.1 178.9 160.3 256 0z'
                };
                ...
                var updateZoom = function () {
                    ...
                    var $wb = container.find('.qtype_cloudpoodll_whiteboard');
                    $wb.css({
                ...
                var createTextBox = function (x, y) {
                    ...
                    container.find('.qtype_cloudpoodll_whiteboard').append($overlay);
                    $textarea.focus();
                ...
                var performSave = function () {
                    ...
                    var vectorControl = $('[name="' + container.data('vectorcontrol') + '"]');
                    var updateControl = $('[name="' + container.data('updatecontrol') + '"]');
                    var draftitemid = container.data('draftitemid');

                    btn.text(self.strings.saving).prop('disabled', true);

                    Ajax.call([{
                        methodname: 'qtype_cloudpoodll_upload_whiteboard_image',
                        args: {
                        ...
                        // Trigger change on controls to notify Moodle form.
                        vectorControl.trigger('change');
                ...
                // --- Auto-save Vector Data ---
                $(board.canvas).on('wb:changed', function () {
                    var vectorData = board.getVectorData();
                    var vectorControlId = container.data('vectorcontrol');
                    if (vectorControlId) {
                        var vectorControl = $('[name="' + vectorControlId + '"]');
                        if (vectorControl.length) {
                            vectorControl.val(vectorData);
                            vectorControl.trigger('change');
                        }
                    }
                ...
                var vectorControlId = container.data('vectorcontrol');
                if (vectorControlId) {
                    var existingJson = $('[name="' + vectorControlId + '"]').val();
                    if (existingJson) {
